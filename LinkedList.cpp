...
LinkedList& LinkedList::operator=(LinkedList rhs) //ф-я присваивания
{
    swap(*this, rhs);   //*обраащение к структуре
    return *this;
...
bool LinkedList::operator!=(const LinkedList& other) const
{
    return !(*this == other); //Не не равны= равны
}
...
void LinkedList::erase(const Node* node) //удаление узла из списка
{
...
void LinkedList::pop_back() //вытащить из конца
{
    erase(last);
}
...
void LinkedList::push_back(const Data& value)  //добавление узла в конце списка
{
...
LinkedList::LinkedList(const LinkedList& source)  //копирование LinkedList
...
LinkedList::Node* LinkedList::node_at(size_t index)   //поиск узла
{
...
std::ostream& operator<<(std::ostream& output, const LinkedList& xs)   // обход и печать списка
{
